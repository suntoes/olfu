...
--
-- Host: 127.0.0.1
-- Generation Time: May 17, 2024 at 02:56 AM
...
CREATE TABLE `reactions` (
  ...
  `PostID` int(11) DEFAULT NULL,
  `Username` varchar(50) DEFAULT NULL,
  `ReactionType` enum('like','dislike') DEFAULT NULL,
  `Timestamp` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
--
-- Triggers `studentenrollment`
--
DELIMITER $$
CREATE TRIGGER `insert_tuition_fee` AFTER INSERT ON `studentenrollment` FOR EACH ROW BEGIN
    DECLARE num_subjects INT;
    DECLARE fee_amount DECIMAL(10,2);

    -- Determine the number of subjects for the class
    SELECT COUNT(*) INTO num_subjects FROM classsubjects WHERE Class_ID = NEW.Class_ID;

    -- Calculate the tuition fee
    SET fee_amount = num_subjects * 4500;

    -- Insert the tuition fee for the student
    INSERT INTO tuition_fees (Student_ID, Class_ID, Fee_Amount) VALUES (NEW.Student_ID, NEW.Class_ID, fee_amount);
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `enroll_student` AFTER INSERT ON `student_profiles` FOR EACH ROW BEGIN
    DECLARE class_id VARCHAR(50);

    -- Determine the class ID based on the student's year level and course
    IF NEW.Year_Level = '1st Year' THEN
        IF NEW.Course = 'Bachelor of Science in Computer Science' THEN
            SET class_id = '1styear_cs';
        ELSEIF NEW.Course = 'Bachelor of Science in Information Technology' THEN
            SET class_id = '1styear_it';
        ELSEIF NEW.Course = 'Bachelor of Science in Entertainment and Multimedia Computing' THEN
            SET class_id = '1styear_emc';
        END IF;
    ELSEIF NEW.Year_Level = '2nd Year' THEN
        IF NEW.Course = 'Bachelor of Science in Computer Science' THEN
            SET class_id = '2ndyear_cs';
        ELSEIF NEW.Course = 'Bachelor of Science in Information Technology' THEN
            SET class_id = '2ndyear_it';
        ELSEIF NEW.Course = 'Bachelor of Science in Entertainment and Multimedia Computing' THEN
            SET class_id = '2ndyear_emc';
        END IF;
    ELSEIF NEW.Year_Level = '3rd Year' THEN
        IF NEW.Course = 'Bachelor of Science in Computer Science' THEN
            SET class_id = '3rdyear_cs';
        ELSEIF NEW.Course = 'Bachelor of Science in Information Technology' THEN
            SET class_id = '3rdyear_it';
        ELSEIF NEW.Course = 'Bachelor of Science in Entertainment and Multimedia Computing' THEN
            SET class_id = '3rdyear_emc';
        END IF;
    ELSEIF NEW.Year_Level = '4th Year' THEN
        IF NEW.Course = 'Bachelor of Science in Computer Science' THEN
            SET class_id = '4thyear_cs';
        ELSEIF NEW.Course = 'Bachelor of Science in Information Technology' THEN
            SET class_id = '4thyear_it';
        ELSEIF NEW.Course = 'Bachelor of Science in Entertainment and Multimedia Computing' THEN
            SET class_id = '4thyear_emc';
        END IF;
    END IF;

    -- Insert the student's enrollment into the studentenrollment table
    INSERT INTO studentenrollment (Student_ID, Class_ID) VALUES (NEW.Student_ID, class_id);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `insert_grades_on_registration` AFTER INSERT ON `student_profiles` FOR EACH ROW BEGIN
    DECLARE class_id VARCHAR(50);
    DECLARE subject_code VARCHAR(10);
    ...
    -- Determine the subject codes based on the class ID
    IF class_id IS NOT NULL THEN
        ...
        read_loop: LOOP
            ...
            -- Insert the student's ID and subject codes into the grades table
